    ...
    for j = 1:num_of_stats_r    %compare each left element with each right element
        ...
        if area_diff < max_area_diff   %check for matching area
            ...
            if orient_diff < max_orient_diff     %check for matching orientation
                l_loco = abs(sqrt(((stats_l(i).Centroid(1) - middle_x).^2 + (stats_l(i).Centroid(2) - middle_y).^2)));
                r_loco = abs(sqrt(((stats_r(j).Centroid(1) - middle_x).^2 + (stats_r(j).Centroid(2) - middle_y).^2)));
                locodiff = abs(r_loco - l_loco);
                if locodiff < max_loco_diff       %check for matching location
